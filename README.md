...
프론트는 **`VITE_API_BASE_URL`이 비어 있으면** `MockEternalReturnClient`로 돌아가고, 응답 `source`는 `cache`.
베이스 URL을 채우면 `RealEternalReturnClient`가 뽑히지만, 아직은 메서드마다 `throwApiError('NOT_IMPLEMENTED', ...)`만 던짐.
